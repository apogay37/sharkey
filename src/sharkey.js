...
const CFG =  {
	...
	LOADCAPTCHA_TIMEOUT   : 30000, //ТАЙМАУТ В МИЛЛИСЕКУНДАХ ФУНКЦИИ ЗАГРУЗКИ КАПЧИ
    UPDATEPOSTS_TIMEOUT   : 30000, //ТАЙМАУТ В МИЛЛИСЕКУНДАХ ФУНКЦИИ ЗАГРУЗКИ НОВЫХ ПОСТОВ С СЕРВЕРА
    ...
    NIGHTMODE             : Store.get('styling.nightmode'),//_CFG.NIGHTMODE,
    AUTOUPDATE_MIN        : 15, //МИНИМАЛЬНЫЙ ИНТЕРВАЛ МЕЖДУ ОБНОВЛЕНИЯМИ В СЕКУНДАХ
    ...
    ISMOBILE              : document.documentElement.clientWidth <= 480 || document.documentElement.clientHeight <= 480,
}
...
Stage('Плашка',                      'abuplashque',    Stage.DOMREADY,     function(){	
    ...
    if(desktop) {
    	...
    	//content = 'Перед тем как создать тред о том, что в каталоге что-то СТРАННОЕ, сбрось кеш браузера ( ctrl + f5 )';
    	content = 'Покупайте красивый мерч Двача в <a href="https://2chmerch.ru/" target="blank">официальном магазине</a>';
   		//content = `Набор окончен, по всем одобренным заявкам были разосланы инвайты на контактные данные, указанные в заявке. `;
    } else {
        //АРХИВ И ГЛАВНАЯ
...
class MediaViewer {
	constructor(data) {
        ...
        if(CFG.MEDIA_NAVBUTTONS) this.btns = MediaViewer.Nav();
	}
