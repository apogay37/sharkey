...
const CFG =  {
    ...
    GETJSON_TIMEOUT       : 30000, //ТАЙМАУТ В МИЛЛИСЕКУНДАХ ФУНКЦИИ ЗАГРУЗКИ НОВЫХ ПОСТОВ С СЕРВЕРА
    ...
    FAVICON_NEWPOSTS      : '<link id="favicon" rel="shortcut icon" href="/makaba/templates/img/favicon_newposts.ico"/>', //Иконка для оповещения о новых постах
    FAVICON_DELETED       : '<link id="favicon" rel="shortcut icon" href="/makaba/templates/img/favicon_deleted.ico"/>', //Иконка для оповещения о удалённом треде
    ...
    ISMOBILE480           : window.matchMedia("only screen and (max-width: 480px)").matches,
}
    ...
    PostQuery.prototype = {
        ...
        fetchPosts: function(param, callback, attempt = 1) {
            ...
            $.ajax(page == -1 ? _.threadJson(board, thread) : _.pageJson(board, page), {
                dataType: 'html',
                timeout: window.config.downloadPostsTimeout,
...
Stage('Наполнение карты постов',                'mapfill',      Stage.INSTANT,     function(){
    ...
    window.PostF = {
        ...
        _appendThread: function(thread) {
            ...
            postshtml += '</div>';
            postshtml += '</div>';
            if(!window.thread.id && postsCount != -1) postshtml += `<div class="thread__missed desktop"><svg xmlns="http://www.w3.org/2000/svg" class="post__btn post__btn_type_expand icon desktop" data-num="${num}"><use xlink:href="#icon__expand"></use></svg> Пропущено  ${postsCount}  постов ${filesCount ? ', ' + filesCount + ' с картинками' :''}.</div>`; 
            postshtml += '</div>';

            if(!window.thread.id && document.getElementById('post-' + num)) {
            	//for expand
            	$(postshtml).insertAfter('#thread-' + num);
            	let newMissMessage = document.getElementById('thread-' + num).getElementsByClassName('thread__missed')[0].cloneNode(true);
            	console.log(newMissMessage);
            	...
            	$(newMissMessage).insertAfter('#thread-' + num + ' .thread__oppost');
        ...
        updatePosts: function(callback) {
            ...
            Post(window.thread.id).fetchPosts({update: true}, function( data ) {
                ...
                that._markPosts(window.thread.id, true);
                if(window.board == 'math') MathJax.Hub.Queue(['Typeset',MathJax.Hub]);
        ...
        _generatePostBody: function(post) {
		    ...
		    postshtml += `<span class="post__time">${window.correctTZ?window.correctTZ(post.date):post.date}</span> </span>
		    			  <span class="post__detailpart">
		    			  	<a href="/${window.thread.board}/res/${parseInt(post.parent)||post.num}.html#${post.num}" id="${post.num}" class="post__reflink">№</a><a href="/${window.thread.board}/res/${parseInt(post.parent)||post.num}.html#${post.num}" id="${post.num}" class="postbtn-reply-href post__reflink"  data-num="${post.num}">${post.num}</a>
		    			 `;
...
Stage('Загрузка Media провайдеров',             'media',        Stage.INSTANT,      function(){
    ...
    Media.add('vocaroo', 'vocaroo.com', "https?://(?:www\\.)?voca\\.ro/([\\w]+)", {id: 1});
    ...
    Media.addGenerator('vocaroo', function(obj, cb){
    	//cb('<div><iframe width="300" height="60" src="https://vocaroo.com/embed/' + obj.id + '?autoplay=0" frameborder="0" allow="autoplay"></iframe><br><a href="https://voca.ro/' + obj.id + '" title="Vocaroo" target="_blank">Посмотреть на Vocaroo &gt;&gt;</a></div>')
...
Stage('Init функции',                     'initialisation',     Stage.DOMREADY,      function(){
	...
	//makaka captcha
	let f = document.querySelectorAll('.postform');
...
Stage('Управление полями загрузки картинок',    'uploadfields', Stage.DOMREADY,     function(){
    ...
    var FormFiles = window.FormFiles = {
        ...
        readAsArrayBufferAsync: function(file) { //to global?
        ...
        swap(id1, id2) {
            ...
            let $boxex = $(`.filer__thumb_c_${id1}`);
            let $boxex2 = $(`.filer__thumb_c_${id2}`);
...
Stage('Обработка и отправка постов на сервер',  'postsumbit',   Stage.DOMREADY,     function(){
    ...
    var validateForm = function(is_qr) {
		...
		//if(!/[а-яё]/i.test($('#shampoo').val()) && window.board == 'b') return $alert('Error occured');
    ...
    $forms.on('submit', function(){
       ...
       //      if(key == -1 || key == -2 ) { //window.board == 'by'
	      //   	try {
	      //   		if(validateForm(form.attr('id') == 'qr-postform')) sendForm(form[0]);
	      //   		//sendForm(form[0]);
	      //   	} catch(e) {
...
Stage('Загрузка автообновления',                'autorefresh',  Stage.DOMREADY,     function(){
    ...
    window.scrollcb_array.push(function(scroll_top){
        ...
        for(var i=0;i<MAutoUpdate.newPosts.length;i++) {
        	let p = document.getElementById('post-body-' + MAutoUpdate.newPosts[i]);
        	if(p === null) continue;
            if(scroll >= p.offsetTop) {
...
Stage('click эвенты',                           'clickevents',  Stage.DOMREADY,     function(){
    ...
    $('.cntnt__right').on('click', '.postbtn-update', (e) => { e.preventDefault(); PostF.updateThread(); })
	...
	document.getElementById('js-nightmode').addEventListener('click', (e) => {
		if ( Store.get('styling.nightmode') ) {
...
Stage('stickers',                          'stickers',  Stage.DOMREADY,     function(){
	...
	var deleteStickers = function(id) {
		if (confirm(`Вы хотите удалить пак?`)) {
			document.querySelector(`#pack-${id}`).remove();
			Store.del('other.sticker.packs.' + id);
		}
	}
...
Stage('Превью постов',                          'postpreview',  Stage.DOMREADY,     function(){
    ...
    //var dForm = document.querySelector('.makaba');
...
Stage('Плашка',                      'abuplashque',    Stage.DOMREADY,     function(){	
    ...
    if(desktop) {
    	//content = 'Подписывайся на главный канал в Телеграме - <a href="https://t.me/dvachannel">@dvachannel</a>. Мемы, новости, смешные видео и коты тебя уже ждут.';
		//content = 'Хочешь узнавать последние новости о жизни вокруг тебя и обсуждать вместе с аноном? Подписывайся на канал в Телеграме <a href="https://t.me/ru2ch_news">@ru2ch_news</a>';
    	//content = 'Привет, анон. В /gd скоро стартует конкурс по созданию игр за 2 недели - TWG! Присоединяйся, чтобы выбрать лучшую <s>игру</s> шизу, или сделай свою.  <a href="https://2ch.hk/gd/res/753261.html">https://2ch.hk/gd/res/753261.html</a>';
    	//content = 'Стартовал традиционный набор в модерку, подать заявку вы можете <a href="/hire.html" target="blank">тут</a>.';
    	//content = 'Покупайте красивый мерч Двача в <a href="https://2chmerch.ru/" target="blank">официальном магазине</a>';
   		content = `Устал скроллить нулевую, но хочется почитать? Аноны с издача написали <a href="//2ch.hk/izd/res/198897.html" target="blank">книги</a> специально для тебя!`;
    } else {
        //АРХИВ И ГЛАВНАЯ
        ...
        //content = '<div style="padding-top: 100px;">Сегодня стартует традиционный набор модераторов на двач. Подать заявку можно <a href="https://2ch.hk/hire.html">тут</a><div> ';
    }
...
Stage('Система лайков',                         'likes',        Stage.DOMREADY,     function(){
    ...
    var like = function(num, el, dislike) {
        ...
        $.ajax( action + '?board=' + window.board + '&num=' + num, {
            dataType: 'json',
            timeout: 5000,
            success: onsuccess,
            error: onerror
...
// function requestMakakaKeys(data) {
//     let f = document.querySelectorAll('.postform');
//     let a = hta(data[2]).split('|');
//     let i3v = xr(a[0],a[1]);
//     let id = document.querySelectorAll('.js-makaka-id');
//     let answer = document.querySelectorAll('.js-makaka-answer');
//     for (let i = 0; i < 2; i++) {
//     	id[i].value = data[0];
//     	answer[i].value = i3v;
//     }
...
// async function requestMakakaKeysTest(data) {
...
//         let a = hta(data[2]).split('|');
//         let i3v = xr(a[0],a[1])
//         let i1 = _.newTempl(`<input type="hidden" id="js-cci" name="makaka_id" value="${data[0]}">`);
//         let i3 = _.newTempl(`<input type="hidden" id="js-ccd" name="makaka_answer" value="${i3v}">`);
//      //    document.getElementById('js-cci').value = data[0];
//     	// document.getElementById('js-ccd').value = i3v;
//     	f[0].append(i1,i3);
//         f[1].append(i1.cloneNode(),i3.cloneNode());
...
const MScreenshotMode = (function () {
	...
	const _helper = {
		blur(num) {
			let id;
			//let nodes = document.getElementById('posts-form').querySelectorAll('.post, .thread__missed'); 
			let nodes = document.getElementsByClassName('cntnt__right')[0].querySelectorAll('.post, .thread__missed'); 
		...
		bindUI() {
		    ...
		    document.querySelector('.cntnt__right').addEventListener('click', (e) => { //posts-form
...
class PopUp {
	...
	_destroy() {
		$del(document.querySelector(`.modal_type_${this.type}`));
	}
	_parseEl() {
		//type check later for menus
		let src;
		let post = this.el.closest('.post__image');
		let m = post.querySelector('.post__file-preview');
...
const MImgSearch = (function () {
	const _helper = {
		bindUI() {
			...
			document.querySelector('.makaba').addEventListener(cl ? 'click' : 'mouseover', (e) => {
			...
			document.querySelector('.makaba').addEventListener('mouseout', (e) => {
				let el = e.target;
				if(!el.classList.contains('js-post-findimg')) return;
				if(PopUp.active) !document.querySelector(".modal_type_imgfindmenu").dispatchEvent(new MouseEvent('mouseout', {bubbles: true}))
				return false;
			});
		}
	}
...
class MediaObject {
	...
	get el()      { return document.querySelector(`#img-${this._post}-${this._md5}`); }
	...
	showOld() {
		let el = null;
		const pview = document.querySelector(`#preview-${this.post}`)
		if(pview) {
			el = pview.querySelector('.post__file-preview');
		} else {
			({ el } = this);
		}
		...
		if(!this.isVideo) {
			...
			if(this.isVideo) {
				...
				if( this.type == 7 ) {
					...
					<video id="html5video" data-state="full" name="media" loop="1" ${Store.get('other.webm_vol',false)?'':'muted="1"'} controls="" autoplay="" height="100%" width="100%"><source class="video" height="100%" width="100%" type="${this.type == 10 ?'video/mp4':'video/webm'}" src="${this.src}"></source></video>
				...
				vid.volume = Store.get('other.webm_vol', 0);
...
class MediaViewer {
	...
	async _open(data) {
        ...
        if (data.isVideo) {
        	...
        	this.main.insertAdjacentHTML('beforeend', '<video id="js-mv-player" class="mv__player" name="media" loop="1" ' + (Store.get('other.webm_vol', false) ? '' : 'muted="1"') + ' controls="" autoplay="" height="100%" width="100%"><source class="video" height="100%" width="100%" type="' + (data.type == 10 ? 'video/mp4' : 'video/webm') + '" src="' + data.src + '"></source></video>');
        	media = this.main.lastElementChild;
        	media.volume = Store.get('other.webm_vol', 0);
...
MediaViewer.Nav = function() {
	...
	`
	const btns = _.newTempl(chtml);
	MediaViewer.container.append(btns)
	const [fwd,back] = [...btns.children];
	let t = null;
	let over = false;
...
async function loadJson(num, board = CFG.BOARD.NAME, page = null) {
    // let url = '';
    // if (page != null) {
    //     url = `/${board}/${page == 0 ? 'index' : page }.json`;
    // } else {
    //     url = `/${board}/res/${num}.json`;
    // }
    const url = page != null ? _.boardJson(board, page) : _.threadJson(board, num);
...
class _Post {
    constructor (postobj) {
        ...
        this.isHidden = false;
        this.el = _.id(`post-body-${this.num}`); //?
        this.thread = _THREADS.get(this.num); // link to Thread obj
        this.refmap = [];
        this.repliesTo = [];
        _POSTS.set(this.num, this);
        ...
        if(message.indexOf('class="post-reply-link"') >= 0) {
            ...
            for (let i = match.length - 1; i >= 0; i--) {
                ...
                if(replyMatch && replyMatch.hasOwnProperty('2')) {
                    ...
                    if(_POSTS.get(num)) _POSTS.get(num).addReply(this.num);
